...
/* ==========================================================================
   ...
   ========================================================================== */
const DOM = {
  mapVehicles: document.getElementById("map-vehicles"),
...
function initContainerCRUD() {
  document.getElementById("edit-container-btn")?.addEventListener("click", async () => {
    ...
    try {
      ...
      addActivityLog("Container Updated", "Shipment status updated successfully.");
      closeModal();
...
/* =========================================================
   GPS Fleet Simulation
========================================================= */

const MAP_VEHICLES = [
  {
    id: "VH-01", label: "Truck Alpha",
    x: 18, y: 48,
    route: [[18,48],[32,44],[50,46],[68,43],[82,46],[68,43],[50,46],[32,44]],
    wi: 0, tx: 32, ty: 44, congested: false,
  },
  {
    id: "VH-02", label: "Truck Bravo",
    x: 72, y: 54,
    route: [[72,54],[58,50],[42,52],[26,49],[42,52],[58,50],[72,54],[85,51]],
    wi: 0, tx: 58, ty: 50, congested: false,
  },
  {
    id: "VH-03", label: "Truck Charlie",
    x: 44, y: 60,
    route: [[44,60],[60,57],[76,59],[88,55],[76,59],[60,57],[44,60],[28,57]],
    wi: 0, tx: 60, ty: 57, congested: false,
  },
  {
    id: "VH-04", label: "Truck Delta",
    x: 14, y: 53,
    route: [[14,53],[30,51],[52,53],[71,49],[86,52],[71,49],[52,53],[30,51]],
    wi: 0, tx: 30, ty: 51, congested: false,
  },
];

// GPS fleet congestion throttle
let _lastCongestionLog = 0;
...
/* ------------------------------------------------------------------
   GPS: renderMapVehicles — module-scope (extracted from renderPortMap)
------------------------------------------------------------------ */
function renderMapVehicles() {
  const el = document.getElementById("map-vehicles");
  if (!el) return;
  el.innerHTML = MAP_VEHICLES.map(v => {
    const color = v.congested ? "#FF5D73" : "#3CF2FF";
    return `<g class="map-vehicle-grp" transform="translate(${v.x.toFixed(2)},${v.y.toFixed(2)})">
      <circle r="3.2" fill="${color}" opacity="0.15"/>
      <circle r="1.7" fill="${color}"/>
      <text dy="-3" text-anchor="middle" font-size="2.1" fill="${color}" opacity="0.85">${v.id}</text>
    </g>`;
  }).join("");
}

/* ------------------------------------------------------------------
   GPS: simulateFleetMovement — module-scope (extracted from simulatePortCongestion)
------------------------------------------------------------------ */
function simulateFleetMovement() {
  MAP_VEHICLES.forEach(v => {
    ...
    if (dist < 1.2) {
      v.wi = (v.wi + 1) % v.route.length;
      v.tx = v.route[v.wi][0];
      v.ty = v.route[v.wi][1];
    } else {
      const spd = 0.15 + Math.random() * 0.06;
      v.x += (dx / dist) * spd;
      v.y += (dy / dist) * spd;
    }
    // proximity congestion check
    v.congested = MAP_VEHICLES.some(o => o.id !== v.id && Math.hypot(o.x - v.x, o.y - v.y) < 6);
  });

  // congestion notification throttled to 1 per 15s
  const now = Date.now();
  if (now - _lastCongestionLog > 15000) {
    const stuck = MAP_VEHICLES.filter(v => v.congested);
    if (stuck.length > 0) {
      _lastCongestionLog = now;
      const ids = stuck.map(v => v.id).join(", ");
      if (typeof addActivityLog === "function") addActivityLog("Fleet Alert", `Congestion: ${ids} proximity detected — rerouting.`);
      if (typeof showToast === "function") showToast("congestion", "Fleet Congestion", `${ids} — proximity alert issued.`);
    }
  }
...
function showToast(type, title, message) {
  ...
  addNotification(type, title, message); // Feed ke Notification Center
  setTimeout(() => {
...
function initStaticUI() {
  ...
  initPortMap();
  renderMapVehicles();
  setInterval(simulateFleetMovement, 80);
  initSmartAlerts();
  initFAB();
...
async function bootstrap() {
  ...
  try {
    ...
    window._nexportSB = supabaseClient;
...
/* ==========================================================================
   ...
   — Panel notifikasi kanan atas, toggle via #notification-toggle
   ========================================================================== */
const notifications = [];
...
function initNotificationCenter() {
  ...
  // BUG FIX: Gunakan optional chaining pada toggleBtn agar tidak crash
  // saat element belum ada di DOM
  document.addEventListener("click", (e) => {
    if (
      panel &&
      !panel.contains(e.target) &&
      !toggleBtn?.contains(e.target)   // ← fix: toggleBtn? bukan toggleBtn
    ) {
...
function addNotification(type, title, message) {
  ...
  // BUG FIX: Guard semua elemen, bukan hanya list, sebelum lanjut
  if (!list) return;

  let cls = "notification-item--info";
  if (type === "danger" || type === "delay" || type === "congestion") {
    cls = "notification-item--danger";
  } else if (type === "success") {
    cls = "notification-item--success";
  }

  const item = document.createElement("div");
  item.className = `notification-item ${cls}`;
  item.innerHTML = `
    <strong>${title}</strong>
    <p>${message}</p>
  ...
  notifications.unshift({ type, title, message, time: Date.now() });

  // Trim list ke 20 item maksimum
  if (notifications.length > 20) {
    list.lastElementChild?.remove();
  }

  // BUG FIX: Guard null sebelum set textContent
  if (count) count.textContent = notifications.length;
  if (total) total.textContent = `${notifications.length} alerts`;
}

/* ==========================================================================
   Activity Log Engine
   — BUG FIX UTAMA: dipindah ke scope global (bukan nested di dalam
     addNotification). Sebelumnya didefinisikan di dalam body addNotification
     sehingga tidak bisa dipanggil dari initContainerCRUD → ReferenceError.
   ========================================================================== */
const activityLogs = [];
...
function addActivityLog(title, description) {
  ...
  list.querySelector(".activity-log-empty")?.remove();
  list.prepend(item);

  activityLogs.unshift({ title, description, time: Date.now() });

  // Trim log ke 20 item maksimum
